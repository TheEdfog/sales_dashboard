# Репозиторий проекта курса Sapiens Solutions: "Аналитика и работа с данными (СУБД Greenplum)"
...
- **`Project_presentation.pptx`**: Презентация PowerPoint с обзором проекта, подготовленная для защиты.
- **`README.md`**: Этот файл с описанием проекта.
- **`SS_project.zip`**: Архив с дашбордами Superset для визуализации витрины данных.
...
### Цель
Проект направлен на создание пайплайна данных для обработки данных о продажах и формирования витрины (`mart_plants_default`) для анализа производительности магазинов. Пайплайн включает загрузку данных из PostgreSQL и CSV-файлов, их хранение в Greenplum, обработку с помощью Airflow и визуализацию в Superset и ClickHouse.
...
2. **Трансформация данных**:
   ...
   - Метрики витрины включают:
     ...
     - Средние показатели (`avg_qty_per_bill`, `btt`, `avg_bill_rub`, `avg_customer_rub`)
...
4. **Визуализация данных**:
   - Дашборды Superset (в `SS_project.zip`) визуализируют метрики витрины, предоставляя интерактивный анализ производительности магазинов.
...
- **Распределение**:
  ...
  - `stores`, `promos`, `promo_types`: Реплицированы (малые таблицы измерений).
  - `traffic`: Случайное распределение.
...
### Дашборды Superset
Дашборды (в `SS_project.zip`) визуализируют данные витрины `mart_plants_default`:
...
1. **Обновление статистики**:
   - После выполнения скриптов в Greenplum нужно обновлять статистику (например, с помощью `ANALYZE`) для оптимизации запросов.
...
5. **Логирование**:
   - Расширить логирование (например, в `f_load_write_log`) для записи количества строк, времени выполнения и деталей ошибок.
6. **Безопасность**:
...
7. **Оптимизация производительности**:
   - Настроить сжатие и партиционирование таблиц Greenplum для больших объемов данных.
...
4. **Настройка Superset**:
   - Импортировать дашборды из `SS_project.zip`.
   - Настроить подключение к ClickHouse для запросов к `mart_plants_default_final_distr`.

## Благодарности
Проект выполнен в рамках курса "Аналитика и работа с данными (СУБД Greenplum)" от Sapiens Solutions. Огромная благодарность преподавателям и менторам за поддержку и руководство на протяжении курса!
